...
macro(_rox_build_lib_file_locations VAR)
    if (LIB_TARGETS)
        ...
        foreach (index RANGE 0 ${RANGE_END} 2)
            ...
            set(LIB_FILE_LOCATION ${LIB_DIR}/${LIB_TARGET_FILE_NAME})
...
macro(_rox_init_third_party_lib_vars)
    ...
    if (NOT LIB_FILE)
        set(LIB_FILE ${CMAKE_STATIC_LIBRARY_PREFIX}${LIB_NAME}${CMAKE_STATIC_LIBRARY_SUFFIX})
    endif ()
    ...
    if (LIB_TRY_FIND_LEN GREATER 0)
        ...
        if (LIB_TRY_FIND_LEN GREATER 1)
            math(EXPR LIB_TRY_FIND_LEN_MINUS_ONE "${LIB_TRY_FIND_LEN}-1")
            foreach (i RANGE 1 ${LIB_TRY_FIND_LEN_MINUS_ONE})
                list(GET LIB_TRY_FIND ${i} LIB_TRY_FIND_TARGET_NAME)
                list(APPEND LIB_TRY_FIND_TARGET_NAMES ${LIB_TRY_FIND_TARGET_NAME})
            endforeach ()
        endif ()
    endif ()
    ...
    if (NOT LIB_BIN_DIR)
        set(LIB_BIN_DIR "${LIB_INSTALL_DIR}/bin")
    endif ()
    ...
    if (NOT LIB_CONFIGURE)
        set(LIB_CONFIGURE <CMAKE> -DCMAKE_INSTALL_PREFIX=${LIB_INSTALL_DIR} -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -H${LIB_SOURCE_DIR}/${LIB_SUBDIR} ${LIB_CMAKE_ARGS})
    endif ()
        ...
        foreach (LIB_TRY_FIND_TARGET_NAME IN LISTS LIB_TRY_FIND_TARGET_NAMES)
            if (TARGET ${LIB_TRY_FIND_TARGET_NAME})
                ...
                if (LIB_TRY_FIND_LIBRARIES AND NOT "${${LIB_TRY_FIND_LIBRARIES}}" STREQUAL "")
                    if (LIB_VERBOSE)
                        message("Target ${LIB_TRY_FIND_TARGET_NAME} NOT found. Trying to create it using the found include dir \"${${LIB_TRY_FIND_INCLUDE_DIR}}\" and libraries \"${${LIB_TRY_FIND_LIBRARIES}}\".")
                    endif ()
...
function(rox_external_lib LIB_NAME)
    ...
    if (ROX_BUILD_THIRD_PARTY_LIBS)
        ...
        if (LIB_SHARED)
            list(APPEND ROX_EXTERNAL_LIB_PATHS ${LIB_BIN_DIR})
            file(GLOB files RELATIVE ${LIB_BIN_DIR} "${LIB_BIN_DIR}/*${CMAKE_SHARED_LIBRARY_SUFFIX}")
            foreach (file IN LISTS files)
                list(APPEND ROX_EXTERNAL_LIB_RUNTIME_FILES "${LIB_BIN_DIR}/${file}")
            endforeach ()
            if (NOT WIN32)
                file(GLOB files LIST_DIRECTORIES false RELATIVE ${LIB_DIR} "${LIB_DIR}/*.*")
                foreach (file IN LISTS files)
                    list(APPEND ROX_EXTERNAL_LIB_RUNTIME_FILES "${LIB_DIR}/${file}")
                endforeach ()
            endif ()
        endif ()
